"""
Period perpendicular crossing targeter for BCR4BP spatial orbits
...
export SpatialPerpP12Targeter
export correct, getMonodromy, getPeriod, getResonantOrbit, homotopy, propagateState

"""
    SpatialPerpP12Targeter(dynamicsModel)

BCR4BP P1-P2 spatial perpendicular crossing period targeter object

# Arguments
- `dynamicsModel::BCR4BP12DynamicsModel`: BCR4BP P1-P2 dynamics model object
"""
struct SpatialPerpP12Targeter
    dynamicsModel::MBD.BCR4BP12DynamicsModel                            # BCR4BP P1-P2 dynamics model object

    function SpatialPerpP12Targeter(dynamicsModel::MBD.BCR4BP12DynamicsModel)
...
"""
function correct(targeter::SpatialPerpP12Targeter, q0::Vector{Float64}, targetP::Float64, synRevs::Int64; tol::Float64 = 1E-11, JTol::Float64 = 2E-3)
    ...
    setFreeVariableMask!(originNode.state, [true, false, true, false, true, false, false])
    ...
    addConstraint!(problem, MBD.BCR4BP12StateConstraint(terminalNode, [2, 4, 6], [0.0, 0.0, 0.0]))
...
"""
function getMonodromy(targeter::SpatialPerpP12Targeter, solution::MBD.BCR4BP12MultipleShooterProblem)
...
- `JTol::Float64`: Jacobian accuracy tolerance (default = 2E-3)
"""
function getResonantOrbit(targeter::SpatialPerpP12Targeter, initialOrbit::MBD.CR3BPPeriodicOrbit, p::Int64, q::Int64; Deltaeps = 0.001, tol = 1E-11, refTol = 1E-11, JTol = 2E-3)
    initialStateGuess::Vector{Float64} = push!(copy(initialOrbit.initialCondition), 0.0)
    refinedSolution::MBD.BCR4BP12MultipleShooterProblem = homotopy(targeter, initialStateGuess, p, q, Deltaeps = Deltaeps, tol = tol, JTol = JTol)
...
- `JTol::Float64`: Jacobian accuracy tolerance (default = 2E-3)
"""
function homotopy(targeter::SpatialPerpP12Targeter, initialStateGuess::Vector{Float64}, p::Int64, q::Int64; Deltaeps::Float64 = 0.001, tol::Float64 = 1E-11, JTol::Float64 = 2E-3)
    ...
    finalSolution::MBD.BCR4BP12MultipleShooterProblem = MBD.deepClone(solution)
    ...
    while epsilon < 1.0
        ...
        try
            ...
            finalSolution = MBD.deepClone(solution)
            converging = true
        catch err
            println("$err: Decreasing step size")
            converging = false
        end
        if converging
            epsilon += Deltaeps
...
"""
function propagateState(targeter::SpatialPerpP12Targeter, q_simple::Vector{Float64}, tSpan::Vector{Float64})
