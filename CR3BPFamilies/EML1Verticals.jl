"""
Script for Earth-Moon CR3BP L1 vertical orbit family

Author: Jonathan LeFevre Richmond
C: 3/12/26
"""
# module EML1Vertical
println("Running EML1Verticals.jl...\n")
...
initialStateGuess::Vector{Float64} = [0.8371, 0, 0.01026, 0, -0.0001582, 0]
tSpanGuess::Vector{Float64} = [0, 2.77]
...
# fullExportCR3BPFamily(family, "FamilyData/CR3BPEML1Verticals.mat", "FamilyData/CR3BPEML1Verticals.csv", :L1Verticals)

# println("\nTesting interpolation...")
# testOrbit::MBD.CR3BPPeriodicOrbit = interpOrbit(targeter, "FamilyData/CR3BPEML1Verticals.csv", "JC", 3.0)
...
# end
# (figure, axis) = set3DPlotParameters(L"Earth-Moon $L_{1}$ Vertical ($JC=%$(round(getJacobiConstant(dynamicsModel, family.initialConditions[plotOrbit]); digits = 4))$)", L"$x$ [ndim]", L"$y$ [ndim]", L"$z$ [ndim]")
# GLMakie.lines!(axis, xData, yData, zData, color = :white, label = L"$L_{1}$ Vertical Orbit")
