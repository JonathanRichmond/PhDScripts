"""
Script for Earth-Moon CR3BP L1 northeastern axial orbit family

Author: Jonathan LeFevre Richmond
C: 3/2/26
"""
# module EML1Axial
println("Running EML1Axials.jl...\n")
...
initialStateGuess::Vector{Float64} = [0.9306, 0, 0, 0, -0.6039685, 0]
tSpanGuess::Vector{Float64} = [0, 3.95]
...
solution2::MBD.CR3BPMultipleShooterProblem = correct(targeter, initialStateGuess+[0, 0, 0, 0, 0, 0.002], tSpanGuess, targetJC-1E-5)
...
x0EndCheck = MBD.BoundingBoxContinuationEndCheck("Initial State", [0.87 0.99; NaN NaN; NaN NaN])
...
# fullExportCR3BPFamily(family, "FamilyData/CR3BPEML1Axials.mat", "FamilyData/CR3BPEML1Axials.csv", :L1Axials)

# println("\nTesting interpolation...")
# testOrbit::MBD.CR3BPPeriodicOrbit = interpOrbit(targeter, "FamilyData/CR3BPEML1Axials.csv", "JC", 3.0)
# println("Test Orbit:\n\tIC:\t$(testOrbit.initialCondition)\n\tP:\t$(testOrbit.period)\n\tJC:\t$(getJacobiConstant(testOrbit))\n")

# println("Plotting orbit...")
# plotOrbit::Int64 = getNumMembers(family)/2
...
# end
# (figure, axis) = set3DPlotParameters(L"Earth-Moon $L_{1}$ Axial ($JC=%$(round(getJacobiConstant(dynamicsModel, family.initialConditions[plotOrbit]); digits = 4))$)", L"$x$ [ndim]", L"$y$ [ndim]", L"$z$ [ndim]")
# GLMakie.lines!(axis, xData, yData, zData, color = :white, label = L"$L_{1}$ Axial Orbit")
