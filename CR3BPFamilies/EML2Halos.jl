...
initialStateGuess::Vector{Float64} = [1.1203862382244514, 0, 0, 0, 0.17604041411386667, 0]
tSpanGuess::Vector{Float64} = [0, 3.4155308796481667]
...
# end
# (figure, axis) = set3DPlotParameters(L"Earth-Moon $L_{2}$ Halo ($JC=%$(round(getJacobiConstant(dynamicsModel, family.initialConditions[plotOrbit]); digits = 4))$)", L"$x$ [ndim]", L"$y$ [ndim]", L"$z$ [ndim]")
