"""
Script for Sun-Venus CR3BP L1 southern halo orbit family

Author: Jonathan LeFevre Richmond
C: 2/9/26
"""
# module SML1Halo
println("Running SVL1Halos.jl...\n")
...
Sun::MBD.BodyData, Venus::MBD.BodyData = systemData.primaryData[1], systemData.primaryData[2]
...
initialStateGuess::Vector{Float64} = [0.9922012915269641, 0, 0, 0, -0.009170865099052702, 0]
tSpanGuess::Vector{Float64} = [0, 3.0639838557271175]
...
VenusEndCheck = MBD.CR3BPPrimarySurfaceContinuationEndCheck(dynamicsModel, 2)
addEndCheck!(continuationEngine, VenusEndCheck)
...
# fullExportCR3BPFamily(family, "FamilyData/CR3BPSVL1Halos.mat", "FamilyData/CR3BPSVL1Halos.csv", :L1Halos)

# println("\nTesting interpolation...")
# testOrbit::MBD.CR3BPPeriodicOrbit = interpOrbit(targeter, "FamilyData/CR3BPSVL1Halos.csv", "JC", 3.0003)
# println("Test Orbit:\n\tIC:\t$(testOrbit.initialCondition)\n\tP:\t$(testOrbit.period)\n\tJC:\t$(getJacobiConstant(testOrbit))\n")

# println("Plotting orbit...")
# plotOrbit::Int64 = getNumMembers(solutions)
...
# end
# (figure, axis) = set3DPlotParameters(L"Sun-Venus $L_{1}$ Halo ($JC=%$(round(getJacobiConstant(dynamicsModel, family.initialConditions[plotOrbit]); digits = 6))$)", L"$x$ [ndim]", L"$y$ [ndim]", L"$z$ [ndim]")
...
# GLMakie.scatter!(axis, getPrimaryState(dynamicsModel, 2)[1], getPrimaryState(dynamicsModel, 2)[2], getPrimaryState(dynamicsModel, 2)[3], color = :orange, markerspace = :data, markersize = 2*Venus.bodyRadius/getCharLength(systemData), label = L"\mathrm{Venus}" => (; markersize = 20))
