"""
Script for Earth-Moon CR3BP L2 vertical orbit family

Author: Jonathan LeFevre Richmond
C: 3/13/26
"""
# module EML2Vertical
println("Running EML2Verticals.jl...\n")
...
targeter = SpatialVerticalJCTargeter(dynamicsModel)

initialStateGuess::Vector{Float64} = [1.1555, 0, 0.009035, 0, 0.000153, 0]
tSpanGuess::Vector{Float64} = [0, 3.518]
...
z0JumpCheck = MBD.BoundingBoxJumpCheck("Initial State", [NaN NaN; 0 1.1; NaN NaN])
addJumpCheck!(continuationEngine, z0JumpCheck)
z0EndCheck = MBD.BoundingBoxContinuationEndCheck("Initial State", [NaN NaN; 0 1.1; NaN NaN])
...
# fullExportCR3BPFamily(family, "FamilyData/CR3BPEML2Verticals.mat", "FamilyData/CR3BPEML2Verticals.csv", :L2Verticals)

# println("\nTesting interpolation...")
# testOrbit::MBD.CR3BPPeriodicOrbit = interpOrbit(targeter, "FamilyData/CR3BPEML2Verticals.csv", "JC", 3.0)
# println("Test Orbit:\n\tIC:\t$(testOrbit.initialCondition)\n\tP:\t$(testOrbit.period)\n\tJC:\t$(getJacobiConstant(testOrbit))\n")

# println("Plotting orbit...")
# plotOrbit::Int64 = 1000
...
# end
# (figure, axis) = set3DPlotParameters(L"Earth-Moon $L_{2}$ Vertical ($JC=%$(round(getJacobiConstant(dynamicsModel, family.initialConditions[plotOrbit]); digits = 4))$)", L"$x$ [ndim]", L"$y$ [ndim]", L"$z$ [ndim]")
# GLMakie.lines!(axis, xData, yData, zData, color = :white, label = L"$L_{2}$ Vertical Orbit")
