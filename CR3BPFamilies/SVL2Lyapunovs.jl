"""
Script for Sun-Venus CR3BP L2 Lyapunov orbit family

Author: Jonathan LeFevre Richmond
C: 2/18/26
"""
# module SVL2Lyap
println("Running SVL2Lyapunovs.jl...\n")
...
(initialStateGuess::Vector{Float64}, tSpanGuess::Vector{Float64}) = getLinearVariation(dynamicsModel, 2, LagrangePoints[2], [-0.0001, 0, 0])
...
# fullExportCR3BPFamily(family, "FamilyData/CR3BPSVL2Lyapunovs.mat", "FamilyData/CR3BPSVL2Lyapunovs.csv", :L2Lyapunovs)

# println("\nTesting interpolation...")
# testOrbit::MBD.CR3BPPeriodicOrbit = interpOrbit(targeter, "FamilyData/CR3BPSVL2Lyapunovs.csv", "JC", 3.0)
...
# end
# (figure, axis) = set2DPlotParameters(L"Sun-Venus $L_{2}$ Lyapunov ($JC=%$(round(getJacobiConstant(dynamicsModel, family.initialConditions[plotOrbit]); digits = 6))$)", L"$x$ [ndim]", L"$y$ [ndim]")
...
# GLMakie.scatter!(axis, getPrimaryState(dynamicsModel, 2)[1], getPrimaryState(dynamicsModel, 2)[2], color = :orange, markerspace = :data, markersize = 2*Venus.bodyRadius/getCharLength(systemData), label = L"\mathrm{Venus}" => (; markersize = 20))
