"""
Script for Earth-Moon CR3BP L2 southern butterfly orbit family

Author: Jonathan LeFevre Richmond
C: 3/13/26
"""
# module EML2But
println("Running EML2Butterflies.jl...\n")
...
targeter = SpatialPerpJCTargeter(dynamicsModel)

initialStateGuess::Vector{Float64} = [1.0708, 0, -0.153, 0, 0.04155, 0]
tSpanGuess::Vector{Float64} = [0, 3.7861]
...
continuationEngine = MBD.JacobiConstantContinuationEngine(solution1, solution2, -1E-5, -1E-2; boundRadiusScaleFactor = 500.0)
z0JumpCheck = MBD.BoundingBoxJumpCheck("Initial State", [NaN NaN; -1 0; NaN NaN])
...
fullExportCR3BPFamily(family, "FamilyData/CR3BPEML2Butterflys.mat", "FamilyData/CR3BPEML2Butterflys.csv", :L2Butterflies)

# println("\nTesting interpolation...")
# testOrbit::MBD.CR3BPPeriodicOrbit = interpOrbit(targeter, "FamilyData/CR3BPEML2Butterflys.csv", "JC", 3.0)
...
# end
# (figure, axis) = set3DPlotParameters(L"Earth-Moon $L_{2}$ Butterfly ($JC=%$(round(getJacobiConstant(dynamicsModel, family.initialConditions[plotOrbit]); digits = 4))$)", L"$x$ [ndim]", L"$y$ [ndim]", L"$z$ [ndim]")
# GLMakie.lines!(axis, xData, yData, zData, color = :white, label = L"$L_{2}$ Butterfly Orbit")
