...
numSegs::Int64 = 28
initialStateGuess::Vector{Float64} = [0.85238285, 0, -0.18039223, 0, 0.24862483, 0]
tSpanGuess::Vector{Float64} = [0, 18.076281/2]
...
z0JumpCheck = MBD.BoundingBoxJumpCheck("Node 1 State", [NaN NaN; -0.4 0; NaN NaN])
...
# end
# (figure, axis) = set3DPlotParameters(L"Earth-Moon $L_{1}$ P7HO1 ($JC=%$(round(getJacobiConstant(dynamicsModel, family.initialConditions[plotOrbit]); digits = 4))$)", L"$x$ [ndim]", L"$y$ [ndim]", L"$z$ [ndim]")
# GLMakie.lines!(axis, xData, yData, zData, color = :white, label = L"$L_{1}$ P7HO1 Orbit")
