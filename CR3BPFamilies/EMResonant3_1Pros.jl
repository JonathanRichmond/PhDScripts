...
initialStateGuess::Vector{Float64} = [0.0680217, 0, 0, 0, 4.66521, 0]
tSpanGuess::Vector{Float64} = [0, 6.33304]
targetJC::Float64 = getJacobiConstant(dynamicsModel, initialStateGuess)
solution1::MBD.CR3BPMultipleShooterProblem = correct(targeter, initialStateGuess, tSpanGuess, targetJC, tol = 5E-11)
...
solution2::MBD.CR3BPMultipleShooterProblem = correct(targeter, initialStateGuess, tSpanGuess, targetJC+1E-4, tol = 5E-11)
...
continuationEngine = MBD.JacobiConstantContinuationEngine(solution1, solution2, 1E-4, 1E-2, tol = 5E-11)
...
# fullExportCR3BPFamily(family, "FamilyData/CR3BPEMResonant3_1Pros.mat", "FamilyData/CR3BPEMResonant3_1Pros.csv", :Resonant3_1Pros)

# println("\nTesting interpolation...")
# testOrbit::MBD.CR3BPPeriodicOrbit = interpOrbit(targeter, "FamilyData/CR3BPEMResonant3_1Pros.csv", "JC", 3.0)
...
# end
# (figure, axis) = set2DPlotParameters(L"Earth-Moon $3:1$ Prograde Resonant Orbit ($JC=%$(round(getJacobiConstant(dynamicsModel, family.initialConditions[plotOrbit]); digits = 4))$)", L"$x$ [ndim]", L"$y$ [ndim]")
# GLMakie.lines!(axis, xData, yData, color = :white, label = L"$3:1$ Prograde Resonant Orbit")
