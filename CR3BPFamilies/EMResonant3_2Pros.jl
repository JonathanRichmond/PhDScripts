"""
Script for Earth-Moon CR3BP 3:2 prograde resonant orbit family
    ***NEED TO CLASSIFY***
...
initialStateGuess::Vector{Float64} = [0.44416, 0, 0, 0, 1.29133, 0]
tSpanGuess::Vector{Float64} = [0, 12.5081]
...
ydot0JumpCheck = MBD.BoundingBoxJumpCheck("Initial State", [NaN NaN; 0 1.5])
addJumpCheck!(continuationEngine, ydot0JumpCheck)
numStepsEndCheck = MBD.NumberStepsContinuationEndCheck(800)
...
# fullExportCR3BPFamily(family, "FamilyData/CR3BPEMResonant3_2Pros.mat", "FamilyData/CR3BPEMResonant3_2Pros.csv", :Resonant2_2Pros)

# println("\nTesting interpolation...")
# testOrbit::MBD.CR3BPPeriodicOrbit = interpOrbit(targeter, "FamilyData/CR3BPEMResonant3_2Pros.csv", "JC", 3.0)
...
# end
# (figure, axis) = set2DPlotParameters(L"Earth-Moon $3:2$ Prograde Resonant Orbit ($JC=%$(round(getJacobiConstant(dynamicsModel, family.initialConditions[plotOrbit]); digits = 4))$)", L"$x$ [ndim]", L"$y$ [ndim]")
# GLMakie.lines!(axis, xData, yData, color = :white, label = L"$3:2$ Prograde Resonant Orbit")
