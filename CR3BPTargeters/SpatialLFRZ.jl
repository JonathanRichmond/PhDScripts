...
export SpatialLFRZTargeter
export correct, getIndividualSolution, getTOF, interpSolution, propagateState

"""
    SpatialLFRZTargeter(dynamicsModel)

CR3BP spatial lunar free return z-position targeter object

# Arguments
- `dynamicsModel::CR3BPDynamicsModel`: CR3BP dynamics model object
"""
struct SpatialLFRZTargeter
    dynamicsModel::MBD.CR3BPDynamicsModel                               # CR3BP dynamics model object

    function SpatialLFRZTargeter(dynamicsModel::MBD.CR3BPDynamicsModel)
...
"""
    correct(targeter, qVector, tVector, numNodes, targetz0, initialh, targeth, targetgamma; tol, JTol)

Return corrected CR3BP multiple shooter problem object

# Arguments
- `targeter::SpatialLFRZTargeter`: CR3BP spatial lunar free return initial z-position targeter object
...
- `targetzdot0::Float64`: Target initial z-velocity
- `initialh::Float64`: Initial altitude [ndim]
...
"""
function correct(targeter::SpatialLFRZTargeter, qVector::Vector{Vector{Float64}}, tVector::Vector{Float64}, numNodes::Int64, targetz0::Float64, initialh::Float64, targeth::Float64, targetgamma::Float64; tol::Float64 = 1E-11, JTol::Float64 = 2E-3)
    ...
    addConstraint!(problem, MBD.CR3BPStateConstraint(nodes[1], [3], [targetz0]))
    addConstraint!(problem, MBD.CR3BPAltitudeConstraint(nodes[1], 2, initialh))
...
"""
function getIndividualSolution(targeter::SpatialLFRZTargeter, family::MBD.CR3BPContinuationFamily, solution::Int64)
    problem = MBD.CR3BPMultipleShooterProblem()
...
"""
function getTOF(targeter::SpatialLFRZTargeter, segments::Vector{MBD.CR3BPSegment})
    TOF::Float64 = 0.0
...
"""
function getTOF(targeter::SpatialLFRZTargeter, solution::MBD.CR3BPMultipleShooterProblem)
    return getTOF(targeter, solution.segments)
end

"""
    interpSolution(targeter, fileName, paramName, paramValue, numNodes, initialh, targeth, targetgamma; choiceIndex, printProgress, tol, JTol)
...
- `JTol::Float64`: Jacobian accuracy tolerance (default = 2E-3)
"""
function interpSolution(targeter::SpatialLFRZTargeter, fileName::String, paramName::String, paramValue::Float64, numNodes::Int64, initialh::Float64, targeth::Float64, targetgamma::Float64; choiceIndex::Int64 = 1, printProgress::Bool = false, tol::Float64 = 1E-11, JTol::Float64 = 2E-3)
    ...
    if abs(familyData[index,paramName]-paramValue) <= 1E-8
        ...
        solution::MBD.CR3BPMultipleShooterProblem = correct(targeter, stateGuesses, timeGuesses, numNodes, initialCondition[3], initialh, targeth, targetgamma; tol, JTol)
    else
        ...
        while (currentError > 1E-8) && (iter <= 20)
            ...
            solution = correct(targeter, stateGuesses, timeGuesses, numNodes, midInitialCondition[3], initialh, targeth, targetgamma; tol, JTol)
...
"""
function propagateState(targeter::SpatialLFRZTargeter, q_simple::Vector{Float64}, tSpan::Vector{Float64})
    propagator = MBD.Propagator()
