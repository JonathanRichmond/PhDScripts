...
q0JumpCheck = MBD.BoundingBoxJumpCheck("Initial State", [0.9 1.0; -1.0 -0.5])
...
HillsSphereEvent = DifferentialEquations.ContinuousCallback(MBD.p1BCR4BP12DistanceCondition, terminateAffect!)
BCR4BPPosManifold::MBD.BCR4BP12Manifold = getManifoldByArclength(orbit, "Unstable", "Positive", 25/get12CharLength(systemData), 50*p)
BCR4BPPosManifold.TOF = propTime
BCR4BPPosManifoldArcs::Vector{MBD.BCR4BP12ManifoldArc} = stopCrashes(BCR4BPPosManifold)
BCR4BPNegManifold::MBD.BCR4BP12Manifold = getManifoldByArclength(orbit, "Unstable", "Negative", 25/get12CharLength(systemData), 50*p)
...
Threads.@threads for a::Int64 in 1:length(BCR4BPManifoldArcs)
    arc::MBD.BCR4BP12Arc = propagateWithEvent(propagator, HillsSphereEvent, real(BCR4BPManifoldArcs[a].initialCondition), [0, propTime], dynamicsModel, [R_H])
...
CR3BPPosManifold::MBD.CR3BPManifold = getManifoldByArclength(compOrbit, "Unstable", "Positive", 25/getCharLength(CR3BPSystemData), 50)
CR3BPPosManifold.TOF = propTime
CR3BPPosManifoldArcs::Vector{MBD.CR3BPManifoldArc} = stopCrashes(CR3BPPosManifold)
CR3BPNegManifold::MBD.CR3BPManifold = getManifoldByArclength(compOrbit, "Unstable", "Negative", 25/getCharLength(CR3BPSystemData), 50)
